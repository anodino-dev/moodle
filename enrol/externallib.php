...
class core_enrol_external extends external_api {
     ...
     * @return external_function_parameters
     */
    public static function unenrol_user_enrolment_ext_parameters() {
        return new external_function_parameters(
            array(
                'userid' => new external_value(PARAM_INT, 'User ID'),
                'courseid' => new external_value(PARAM_INT, 'Course ID')
     ...
     * @return array An array consisting of the processing result, errors.
     */
    public static function unenrol_user_enrolment_ext($userid,$courseid) {
        global $CFG, $DB, $PAGE;
        
        $params = self::validate_parameters(self::unenrol_user_enrolment_ext_parameters(), [
            'userid' => $userid,
            'courseid' => $courseid            
        ]);
        ...
        $enrol = $DB->get_record('enrol', ['courseid' => $params['courseid'],'enrol' => 'manual'], '*', MUST_EXIST);
        if ($enrol) {
        ...
        $userenrolment = $DB->get_record('user_enrolments', ['userid' => $params['userid'],'enrolid' => $enrolid ], '*');
        if ($userenrolment) {
            $userid = $userenrolment->userid;
            $course = get_course($courseid);
            $context = context_course::instance($course->id);
     ...
     * @return external_description
     */
    public static function unenrol_user_enrolment_ext_returns() {
        return new external_single_structure(
            array(
                'result' => new external_value(PARAM_BOOL, 'True if the user\'s enrolment was successfully updated'),
                'errors' => new external_multiple_structure(
